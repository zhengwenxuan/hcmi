...
public class GetHisResControlFZ {
			...
			while (rs.next()) {
				...
				if(hisstatus){
					String updatesql="update examinfo_charging_item set pay_status='Y' where id in"
							+ "( select b.id from charging_detail_single a,examinfo_charging_item b,charging_summary_single c  "
							+ "where a.summary_id='"+summary_id+"' and b.charge_item_id=a.charging_item_id and b.examinfo_id='"+exam_id+"' "
									+ "and c.is_active='Y' and c.exam_id=b.examinfo_id  and c.charging_status!='Y' and c.req_num='"+req_num+"')";
	...
	private boolean getHisStatus(String url,String exam_num,String reqno,String logname){
		...
		try {
			...
			if ("1".equals(sfzt)) {
				...
				c.setString(1, reqno);							
				c.setInt(2, Integer.valueOf(eu.getPatient_id()));
				c.setString(3, czyxm);
