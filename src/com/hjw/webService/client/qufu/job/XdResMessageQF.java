...
import com.hjw.webService.client.qufu.job.xdbean.ResXdMessageQF;
import com.hjw.webService.client.qufu.job.xdbean.RetXdCustomeQF;
import com.hjw.webService.client.qufu.job.xdbean.RetXdItemQF;
   ...
   static{
	...
	public ResultPacsBody getMessage(String url, String exam_num, String req_code) {
		...
		try {
			...
			request.getRequestHeader().setReceiver("2.16.840.1.113883.4.487.4.46.9");//ECG	心电系统
			...
			requestBody.setPAYLOADTYPE("XDREPORT");//检查报告
...
//			requestBody.setEXTEND_CONDITION("");
			...
			if("0".equals(response.getResponseHeader().getErrCode())) {
				...
				if(response.getResponseBody().length() > 20) {
					ResXdMessageQF rpm = new ResXdMessageQF(response.getResponseBody(), true);
					RetXdCustomeQF rc = rpm.getRetXdCustome();
					try {
						...
						String datatime = rc.getEffectiveTime().substring(0,8);
						
						RetXdItemQF rpi = rc.getXdItem();
						pacsResult.setClinic_diagnose(rpi.getChargingItem_jl());
						pacsResult.setStudy_type("EL");
						...
						if (rpi.getBase64_bg().length() > 10) {
							String file_img = PacsPictureDecodeBase64Util.decodeBase64PDF(exam_num, req_code, datatime, rpi.getBase64_bg());
							pacsResult.setIs_tran_image(0);
							pacsResult.setReport_img_path(file_img);
						} else {
							pacsResult.setIs_tran_image(0);
						}
			...
			ex.printStackTrace();
			configService.insert_message_log(til.getId(), til.getMessage_seq_code(), "心电调用webservice错误:"+com.hjw.interfaces.util.StringUtil.formatException(ex));
			til.setFlag(1);
			rb.getResultHeader().setTypeCode("AE");
			rb.getResultHeader().setText("心电调用webservice错误");
		}
