...
public class ExamRequest_PACS_ByExam_num {
	...
	public ExamRequest_PACS_ByExam_num(ThridInterfaceLog til) {
		this.til = til;
	}
	...
	public Response checkInfoQuery(RequestBody requestBody) {
		configService.insert_message_log(til.getId(), til.getMessage_seq_code(), "====================================PACSByExam_num====================================");
		...
		if(eu == null || eu.getId()==0) {
			...
			if(pacsSendList.size() == 0 && "".equals(noPayItems)){
				...
				response.getResponseHeader().setErrCode("5000");
				response.getResponseHeader().setErrMessage("体检号【"+eu.getExam_num()+"】没有需要发送申请的影像科室项目!");
