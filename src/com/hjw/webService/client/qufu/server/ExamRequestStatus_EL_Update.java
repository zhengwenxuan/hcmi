...
import com.hjw.webService.client.qufu.job.XdResMessageQF;
...
public class ExamRequestStatus_EL_Update {
	...
	public ExamRequestStatus_EL_Update(ThridInterfaceLog til) {
		this.til = til;
	}
	...
	public Response checkInfoQuery(RequestBody requestBody) {
		configService.insert_message_log(til.getId(), til.getMessage_seq_code(), "====================================Status_EL_Update====================================");
		...
		if("0".equals(requestBody.getFLAG())) {//0-未检查
			...
			WebserviceConfigurationDTO wcd = webserviceConfigurationService.getWebServiceConfig("XD_READ");
				...
				public void run() {
					...
					try {
						...
						if("AA".equals(rb.getResultHeader().getTypeCode())) {
							configService_inner.insert_message_log(til.getId(), til.getMessage_seq_code(), "更新状态后主动获取EL报告结果-成功");
						} else {
							configService_inner.insert_message_log(til.getId(), til.getMessage_seq_code(), "更新状态后主动获取EL报告结果-失败");
						}
					} catch (Throwable e) {
						configService_inner.insert_message_log(til.getId(), til.getMessage_seq_code(), "更新状态后主动获取EL报告结果-异常："+com.hjw.interfaces.util.StringUtil.formatException(e));
						e.printStackTrace();
					}
				}
			}).start();
