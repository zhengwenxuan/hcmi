...
public class ExamRequest_PACS_ByTime {
	...
	public ExamRequest_PACS_ByTime(ThridInterfaceLog til) {
		this.til = til;
	}
	...
	public Response checkInfoQuery(RequestBody requestBody) {
		configService.insert_message_log(til.getId(), til.getMessage_seq_code(), "====================================PACSByTime====================================");
		...
		if(list == null || list.isEmpty()) {
			...
			for(ExamInfoUserDTO eu : list) {
				...
				sb.append(" and d.dep_num not in ('EL','BLK') ");
				...
				if(pacsSendList.size() == 0 && "".equals(noPayItems)){
					configService.insert_message_log(til.getId(), til.getMessage_seq_code(), "体检号【"+eu.getExam_num()+"】没有需要发送申请的影像科室项目!");
...
//					examRequest.setMOBILE(eu.get);//家属联系电话
			...
			if(responseBody.getExamRequest().isEmpty()) {
				response.getResponseHeader().setErrCode("5000");
				response.getResponseHeader().setErrMessage("时间段【"+requestBody.getSTARTTIME()+"-"+requestBody.getENDTIME()+"】内无影像项目数据。");
				configService.insert_message_log(til.getId(), til.getMessage_seq_code(), "时间段【"+requestBody.getSTARTTIME()+"-"+requestBody.getENDTIME()+"】内无影像项目数据。");
			} else {
