...
public class AutoGetClinicItemMessageQH {
		...
		for(ITEM_INFO Info : clinic.getITEM_LIST().getITEM_INFO()) {
			...
			try {
				...
				if(!"".equals(Info.getITEM_CODE())) {
					...
					String is_VALID = Info.getIS_VALID();//是否停用 1 是， 2 否
					...
					if(!rs.next()) {
						String insert_clinic_sql = "insert into his_clinic_item([item_class],[item_code],[item_name],[input_code]"
								...
								+ ",'"+Info.getSPEC_NAME()+"','"+Info.getCHARGE_UNIT()+"','"+Info.getOP_CRG_TYPE()+"','','"+DateTimeUtil.getDateTime()+"','"+DateTimeUtil.getDateTime()+"','"+Info.getUNIT_PRICE()+"','"+is_VALID+"')";
		...
		try {
			...
			while (charging_item_rs.next()) {
				...
				//String hiscodeClass = charging_item_rs.getString("hiscodeClass");
					...
					sql = "	SELECT price,is_active FROM his_clinic_item where	item_code = '" + charging_item_rs.getString("his_num") +"' ";
					...
					while (clinic_item_rs.next()) {//his_num对应诊疗项目
						...
						if (amount != charging_item_rs.getDouble("amount")) {
							...
							if(is_active.equals("Y")){//诊疗项目是否停用
								...
								String ecisql = " select eci.id from examinfo_charging_item eci,exam_info e where e.id=eci.examinfo_id and eci.charge_item_id='" + charging_item_rs.getLong("id") + "' and eci.isActive='Y  and eci.exam_status !='Y' ";
								...
								String delsetchargingsql = " delete  set_charging_item where  ";
