...
public class PACSDELSendMessageQH {
	...
	public PACSDELSendMessageQH(PacsMessageBody pacsmessage){
	...
	private ResultHeader pacsQHSendMessage(String url,PacsComponents compts,String logname) {
		...
		try {
			...
			bufferXml.append("				<id root=\"2.16.156.10011.1.24\" extension=\""+compts.getReq_no()+"\"/>   ");
			...
			bufferXml.append("					<signatureText>"+pacsmessage.getDoctor().getDoctorName()+"</signatureText> ");
			bufferXml.append("					<assignedEntity classCode=\"ASSIGNED\"> ");
			bufferXml.append("						<!--医务人员ID-->   ");
			bufferXml.append("						<id root=\"2.16.156.10011.1.4\" extension=\""+pacsmessage.getDoctor().getDoctorCode()+"\"/>  ");
			bufferXml.append("						<assignedPerson classCode=\"PSN\" determinerCode=\"INSTANCE\">  ");
			bufferXml.append("							<!--申请单开立者-->   ");
			bufferXml.append("							<name>"+pacsmessage.getDoctor().getDoctorName()+"</name>   ");
			...
			bufferXml.append("							<id root=\"2.16.156.10011.1.26\" extension=\""+pacsmessage.getDoctor().getDept_code()+"\"/> ");
			...
			if(!"".equals(pacsmessage.getCustom().getPersonidnum()) && pacsmessage.getCustom().getPersonidnum()!=null) {
				bufferXml.append("									<id root=\"2.16.156.10011.1.3\" extension=\""+ pacsmessage.getCustom().getPersonidnum()+"\"/>");
			}else {
				bufferXml.append("									<id root=\"2.16.156.10011.1.3\" />");
			}
			bufferXml.append("									<name use=\"L\">" + pacsmessage.getCustom().getName() + "</name>   ");
			bufferXml.append("									<!--性别-->																														");
			
			TranLogTxt.liswriteEror_to_txt(logname, "性别代码==" + pacsmessage.getCustom().getSexcode() + "性别=="+pacsmessage.getCustom().getSexname());
			
			bufferXml.append("									<administrativeGenderCode code=\""+pacsmessage.getCustom().getSexcode()+"\" codeSystem=\"2.16.156.10011.2.3.3.4\" displayName=\""+pacsmessage.getCustom().getSexname()+"\" codeSystemName=\"生理性别代码表（GB/T 2261.1）\"/>  ");
			bufferXml.append("									<!--出生日期--> ");
			bufferXml.append("									<birthTime value=\""+pacsmessage.getCustom().getBirthtime()+"\"/> ");
			...
			if(!"".equals(pacsmessage.getCustom().getArch_num()) && pacsmessage.getCustom().getArch_num()!=null) {
				bufferXml.append("										<id root=\"2.16.156.10011.1.2\" extension=\""+pacsmessage.getCustom().getArch_num()+"\"/>   ");
			}else {
			...
			TranLogTxt.liswriteEror_to_txt(logname, "类别== compts.getPacsComponent().get(0).getExam_class()==" + compts.getPacsComponent().get(0).getExam_class()+ "\r\n");
			
			if("BL".equals(compts.getPacsComponent().get(0).getExam_class())) {
				extXml = pacsExtMessageBL(logname);
			}else {
				extXml = pacsExtMessageJC(logname);
			}
	...
	private String pacsExtMessageJC(String logname) {
		...
		bufferXml.append("      <SEQ_NO>001</SEQ_NO>  ");
		bufferXml.append("      <EXM_TABOO_CODE>TJ001</EXM_TABOO_CODE>   ");
		bufferXml.append("      <EXM_TABOO_NAME>体检信息</EXM_TABOO_NAME> ");
		bufferXml.append("    </EXM_TABOO_INFO> ");
		bufferXml.append("  </EXM_TABOO_LIST>   ");
		...
		List<PacsComponents> components = pacsmessage.getComponents();
		for (int i = 0; i < components.size(); i++) {
			PacsComponents pacscoms = new PacsComponents();
			pacscoms = components.get(i);
			List<PacsComponent> component = pacscoms.getPacsComponent();
			for (int j = 0; j < component.size(); j++) {
				
				bufferXml.append("    <EXM_INFO>");
				bufferXml.append("      <SEQ_NO>"+i+"</SEQ_NO>  ");
				
				String lisCode = getSpecimenItemPacs(component.get(j).getHis_num(),logname);
				String[] lisCodeResult = lisCode.split("_");
				
				if(lisCode.length()>3) {
					bufferXml.append("      <EXM_CLASS_CODE>"+lisCodeResult[1]+"</EXM_CLASS_CODE>");
					bufferXml.append("      <EXM_CLASS_NAME>"+lisCodeResult[0]+"</EXM_CLASS_NAME>   ");
				}else {
				...
				bufferXml.append("      <EXM_ITEM_CODE>"+component.get(j).getItemCode()+"</EXM_ITEM_CODE>  ");
				bufferXml.append("      <EXM_ITEM_NAME>"+component.get(j).getItemName()+"</EXM_ITEM_NAME>   ");
	 ...
	 * @return
	 */
	private String pacsExtMessageBL(String logname) {
		
		StringBuffer bufferXml = new StringBuffer();
		
		bufferXml.append(" <RESPONSE>   ");
		bufferXml.append(" <REQ_MSGID></REQ_MSGID>  ");
