 ...
 * @version V2.0.0.0
 */
public class FEETermSendMessageHjw {
	...
	public FEETermSendMessageHjw(String personid,String accnum) {
		this.accnum = accnum;
	...
	public FeeResultBody getMessage(String url, String logname) {
		...
		try {
			...
			try {
				...
				TranLogTxt.liswriteEror_to_txt(logname, "req:" + accnum + " - prc_tijian_bingrenxx_300("
						+ "'"+accnums.getCom_name()+"',"
						+ "'',"
						+ "'"+date+"',"
						+ "'',"
						+"'"+accnums.getAcc_num()+"',?,?)");
				c.setString(1, accnums.getCom_name());
				c.setString(2, "");
				c.setDate(3, date);
				c.setString(4, "");//(雷双：prm_shenfenzh in varchar2 身份证号  这个取消掉，不给HIS传了。)
				c.setString(5, accnums.getAcc_num());
				...
				TranLogTxt.liswriteEror_to_txt(logname,
						"res:" + accnum + ":存储过程 prc_tijian_bingrenxx_300 执行成功!");
			} catch (Throwable ex) {
				TranLogTxt.liswriteEror_to_txt(logname, "res:" + accnum
			...
			if (!rs1.next()) {
				String insertsql = "insert into Pat_Info(patID,vipid,patName,sex,birth,ptFlag,regDate,req_no,ChargeFlag) values('" 
						+ accnum + "','" + accnums.getBatch_num()+"-"+accnums.getCom_name()+ "','"
