 ...
 * @version V2.0.0.0
 */
public class LISSendMessageGH300 {
	...
	public LISSendMessageGH300(LisMessageBody lismessage) {
			...
			for (LisComponents lcs : this.lismessage.getComponents()) {
				try {
					...
					TranLogTxt.liswriteEror_to_txt(logname,
							...
							list.add(ap);
					if("0".equals(c.getString(18))) {
						TranLogTxt.liswriteEror_to_txt(logname, "req:" + "修改样本为已采样，以使下次新生成条码号");
						dataService.updateSampleExamDetailByExamid(eu.getId(), sd.getId().longValue(), "Y");
					}
				} catch (Exception ex) {
