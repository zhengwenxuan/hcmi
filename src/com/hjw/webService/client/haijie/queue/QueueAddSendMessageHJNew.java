 ...
 * 
     ...
     * @version V2.0.0.0
 */
public class QueueAddSendMessageHJNew {
	...
	public QueueResBody getMessage(String url,String tokenurl,QueueAddBean eu,String logname) {
		...
		try {
			...
			if(success) {
				//org.json.JSONObject resultValues = jsonObject.optJSONObject("resultValues");//返回值 未改之前
				//org.json.JSONObject queue = resultValues.optJSONObject("queue");//返回值 未改之前
				org.json.JSONObject queue = jsonObject.optJSONObject("queue");
				if(queue != null && !"".equals(queue) && !"null".equals(queue)) {
					...
					//rb.setRestext("对方返回："+resultValues.optString("message"));  //返回值 未改之前
					rb.setRestext("对方返回："+jsonObject.optString("message"));
					rb.setRescode("AA");
					rb.setIdnumber("");
				}
			} else {
