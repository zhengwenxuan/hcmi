...
import com.hjw.webService.client.qiyang.bean.type_rec_cur_fee_team;
...
public class TTHisSendMessageQY {
	...
	public TTHisSendMessageQY(String personid,String accnum) {
		this.accnum = accnum;
	...
	private ResultHeader getString(String url, String logname) {
		...
		rh = Pro_send_sick_clinic_detail(wcf,cb,tblist);
		System.err.println(rh.getTypeCode()+"=========成功失败标识111========");
		if(rh.getTypeCode().equals("0")){
			System.err.println(rh.getTypeCode()+"=========成功失败标识222========");
			...
			try {
				System.err.println(his_sql+"=========插入表========");
				System.err.println(rh.getTypeCode()+"=========成功失败标识333========");
				connection = jdbcQueryManager.getConnection();
				connection.createStatement().executeUpdate(his_sql);
				
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				System.err.println(rh.getTypeCode()+"=========成功失败标识444========");
				e.printStackTrace();
			}finally {
				try {
					if(connection != null) {
			...
			rh.setTypeCode("AA");
			rh.setText("his成功");
			System.err.println(rh.getTypeCode()+"=========成功失败标识555========");
		}else{
			rh.setTypeCode("AE");
			rh.setText("his结账失败");
		}
	...
	private ResultHeader Pro_send_sick_clinic_detail(WebserviceConfigurationDTO wcf, ComAccBean cb, List<TeamAccBeanHK> tblist) {
		...
		try {
			...
			System.err.println(cb.getBatch_num()+"=批次==");
			 ...
			 call.registerOutParameter(12, java.sql.Types.LONGVARCHAR);//--his记账流水号
			
			 call.registerOutParameter(13, java.sql.Types.FLOAT);// --错误编码(0成功，-1失败)
			
			 call.registerOutParameter(14,java.sql.Types.LONGVARCHAR);// --错误信息
			
			 
			 call.execute();
			 
			 rh.setSourceMsgId(call.getString(12));
			 rh.setTypeCode(call.getString(13));
			 rh.setText(call.getString(14));
