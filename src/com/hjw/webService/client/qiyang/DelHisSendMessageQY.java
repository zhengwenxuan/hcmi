...
public class DelHisSendMessageQY {
	...
	public DelHisSendMessageQY(UnFeeMessage feeMessage){
			 ...
			 for (int i = 0; i < listhisno.size(); i++) {
					if(!listhisno.get(i).getHis_req_no().equals("") && !listhisno.get(i).getHis_req_no().equals("null")){
						...
						if(rh.getTypeCode().equals("0") && rh.getTypeCode() != null){
							...
							TranLogTxt.liswriteEror_to_txt(logname, "res:申请单号" + feeMessage. getREQ_NOS().getREQ_NO().get(0) +"流水号:"+listhisno.get(i).getHis_req_no() + "成功失败标识:"+rh.getTypeCode()+"\r\n");
						}else{
							rh.setTypeCode("AE");
	...
	public List<ZlReqHisItemDTO>  gethisreqno(ExamInfoUserDTO eu, UnFeeMessage unfee,String logname) {
		String his_req_no="";
		...
		String sql = "select distinct zrhi.his_req_no from charging_summary_single css,charging_detail_single cds,zl_req_his_item zrhi "
		 		...
		 		+ " and css.req_num='"+feeMessage.getREQ_NOS().getREQ_NO().get(0)+"' and css.exam_id='"+eu.getId()+"' and zrhi.exam_num='"+feeMessage.getEXAM_NUM()+"'";
		
		 
		TranLogTxt.liswriteEror_to_txt(logname, "查询his流水号:" + sql + "\r\n");
		 hisno_list  =  this.jdbcQueryManager.getList(sql, ZlReqHisItemDTO.class);
		 
		  return hisno_list;
	}

	public ResultHeader Pro_send_sick_clinic_detail_cancel(ExamInfoUserDTO eu, WebserviceConfigurationDTO wcf,UnFeeMessage fee, String his_req_no) {
		ResultHeader rh = new ResultHeader();
