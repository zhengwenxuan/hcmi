 ...
 * 
     ...
     * @version V2.0.0.0
 */
public class LISDELSendMessageZLLF {
	...
	public LISDELSendMessageZLLF(LisMessageBody lismessage){
		...
		try {
			...
			for (LisComponents pcs : lismessage.getComponents()) {
				...
				try {
					...
					lisdelfalg=false;
					TranLogTxt.liswriteEror_to_txt(logName,
							"res:" + lismessage.getMessageid() + ": 1、lis调用"+table+".CancelSendRequest错误" + com.hjw.interfaces.util.StringUtil.formatException(ex));
				}				
				 if(!lisdelfalg){
					break;					
				  }		
