 ...
 * 
     ...
     * @version V2.0.0.0
 */
public class MSGSendMessageTJ180 {
			...
			try {
				...
				while (rs.next()) {
					...
					if((mess!="")&&(mess.trim().length()>0)){
						...
						MSGResBean resdah = new MSGResBean();
						resdah = (MSGResBean) JSONObject.toBean(jsonobject, MSGResBean.class);
						TranLogTxt.liswriteEror_to_txt(logname, "res:" + result +"\r\n");
						if ("TRUE".equals(resdah.getR().toUpperCase())) {
							rb.setRestext("");
							rb.setRescode("AE");
							rb.setIdnumber("");
							tjtmpconnect.createStatement().execute("update crm_sms_send set sms_status=2,sms_fal_notice='' where id='"+smssendid+"'");
						}else{
							rb.setRestext("");
							rb.setRescode("AA");
							rb.setIdnumber("");
							tjtmpconnect.createStatement().execute("update crm_sms_send set sms_status=1,sms_fal_notice='"+resdah.getI()+"' where id='"+smssendid+"'");	
						}
						}
					}
					}catch(Exception ex){
