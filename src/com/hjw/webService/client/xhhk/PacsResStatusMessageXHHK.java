...
public class PacsResStatusMessageXHHK{
	...
	public String getMessage(String strbody, String logname) {
		...
		ExamInfoUserDTO ei = configService.getExamInfoForReqNum(applyStatus.get(0).getApplyNo());
		List<String> req_nums = new ArrayList<>();
		req_nums.add(applyStatus.get(0).getApplyNo());
		String statuss = applyStatus.get(0).getApplyStatus()+"";
		...
		if(applyStatus.get(0).getApplyNo().length() <= 0 && "".equals(applyStatus.get(0).getApplyNo())|| applyStatus.get(0).getApplyNo().length()<=0  &&"".equals(applyStatus.get(0).getApplyNo())){
			...
			if("1".equals(statuss)||("5".equals(statuss))||("3".equals(statuss))||("4".equals(statuss)) || ("6".equals(statuss))){
				statuss="C";
				
				this.configService.setExamInfoChargeItemPacsStatus(applyStatus.get(0).getApplyNo(),  statuss);
				response.setCode(0);
				response.setMsg("状态通知成功");
			}else if(("2".equals(statuss))){//已检查
				if("Z".equals(ei.getStatus())) {
					
					response.setCode(1);
					response.setMsg("体检中心已总检，不可撤销");
