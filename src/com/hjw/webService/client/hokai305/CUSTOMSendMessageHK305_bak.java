...
import com.hjw.webService.client.hokai305.bean.QueryCustomer;
import com.hjw.webService.client.hokai305.bean.ResCustomBeanHK305;
import com.synjones.framework.persistence.JdbcQueryManager;

public class CUSTOMSendMessageHK305_bak {
	private static JdbcQueryManager jdbcQueryManager;
	private static ConfigService configService;
	private Custom custom=new Custom();
	private ResCustomBeanHK305 rb1= new ResCustomBeanHK305();
	...
	public CUSTOMSendMessageHK305_bak(Custom custom){
		this.custom=custom;
	}
	...
	public ResultBody getMessage(String url,String configvalue,String logname) {
		...
		if(examid<=0){
			...
			try {				
				ResCustomBeanHK305 rcb= new ResCustomBeanHK305();
	...
	private void setSearchString(String url,String statusCode,String logname){
		...
		sb0.append("	<identityCode code=\"11\" displayName=\"军人\"/> ");
		sb0.append("	<!--出生日期(1..1)-->");
	 ...
	 * @return
	 */
	public ResCustomBeanHK305 searchPatid(String url,String logname){	
		 
		 ArrayList<QueryCustomer> customerList = new ArrayList<QueryCustomer>();
		...
		sb0.append("  <id>");
		if(!this.custom.getID_NO().equals("") || this.custom.getID_NO() !=null){
			
			sb0.append("  <item extension=\""+this.custom.getID_NO()+"\" root=\"2.16.156.10011.1.3\"/> ");
		}else{
			sb0.append("  <item extension=\""+this.custom.getCLINIC_NO()+"\" root=\"2.16.156.10011.0.2.2\"/> ");
		...
		sb0.append("  <administrativeGenderCode code=\""+sexCode+"\" displayName=\""+this.custom.getSEX()+"\"/> ");
		sb0.append("  <birthTime value=\""+this.custom.getDATE_OF_BIRTH()+"\" /> ");
		...
		TranLogTxt.liswriteEror_to_txt(logname,"request:"+sb0.toString());
		 ...
		 if(result.trim().length()>0) {
			 try{
					...
					if(!rb1.getFaly() && patid.equals("") && patid.length()<0){
						...
						for (int i = 0; i < subject1List.size(); i++) {
							...
							//患者id
							customer.setPersionid(subject1List.get(0).selectSingleNode("abc:registrationRequest/abc:subject1/abc:patient/abc:id/abc:item[@root='2.16.156.10011.0.2.2']/@extension").getText());
							//身份证号
							customer.setId_no(subject1List.get(0).selectSingleNode("abc:registrationRequest/abc:subject1/abc:patient/abc:patientPerson/abc:id/abc:item[@root='2.16.156.10011.1.3']/@extension").getText());
							//姓名
							customer.setUsername(subject1List.get(0).selectSingleNode("abc:registrationRequest/abc:subject1/abc:patient/abc:patientPerson/abc:name/@value").getText());
							//性别
							customer.setSex(subject1List.get(0).selectSingleNode("abc:registrationRequest/abc:subject1/abc:patient/abc:patientPerson/abc:administrativeGenderCode/@code").getText());
							//身份类型  军人
							customer.setIdentityCode(subject1List.get(0).selectSingleNode("abc:registrationRequest/abc:subject1/abc:patient/abc:patientPerson/abc:identityCode/@displayName").getText());
							//年龄
							...
							//地址
							customer.setAddr(subject1List.get(0).selectSingleNode("abc:registrationRequest/abc:subject1/abc:patient/abc:patientPerson/abc:addr/abc:part[@type='SAL']/@value").getText());
					...
					rb1.setCustomerList(customerList);
					}catch(Exception ex){
						try{
							InputStream is = new ByteArrayInputStream(result.getBytes("utf-8"));
							Map<String, String> xmlMap = new HashMap<>();
