 ...
 * 
     ...
     * @version V2.0.0.0
 */
public class FEEResMessageHK305{
   private static JdbcQueryManager jdbcQueryManager;
   private static ConfigService configService;
   private static String hisamount;
...
//			String exam_num=ei.getExam_num();
				...
				try {
					...
					String sb1 = "select css.exam_id,css.id,css.charging_status,ei.exam_num,amount1,amount2 from charging_summary_single css,exam_info ei where css.exam_id=ei.id "
							+ "and ei.is_Active='Y' "
					...
					while (rs.next()) {
						...
						double amount2 = rs.getDouble("amount2");//折后金额
						double amount1 = rs.getDouble("amount1");//原金额
						//记录状态 0-输入尚未生效的项目；1-收费记录或记帐记录;2-退费记录或销帐记录;3-已被退费的收费记录或已被销帐的记帐记录
						if("1".equals(statuss)){
							if("R".equals(charging_status)) {
								//判断his 返回收费金额是否和 体检收费金额一致
								double parseDouble = Double.parseDouble(hisamount);
								TranLogTxt.liswriteEror_to_txt(logname, "his金额和体检金额校验："+hisamount+"===="+amount2);
								TranLogTxt.liswriteEror_to_txt(logname, "his金额和体检金额校验："+(parseDouble==amount2));
								if(parseDouble==amount2){
		...
		sb.append("	<id  extension=\""+rb.getReqno()+"\"/>\n");
		sb.append("		<!--请求消息ID-->\n");
	...
	private ResFeeStatusBeanHK getreqNo(String xmlstr){
		...
		try{
			...
			//reqno.setPersionid(document.selectSingleNode("abc:POOR_IN200901UV/abc:controlActProcess/abc:subject/abc:observationReport/abc:recordTarget/abc:patient/abc:id/abc:item/@extension").getText());// 获取根节点;
			reqno.setReqno(document.selectSingleNode("abc:POOR_IN200901UV/abc:controlActProcess/abc:subject/abc:observationRequest/abc:id/abc:item[@root='2.16.156.10011.1.24']/@extension").getText());
			reqno.setStatus(document.selectSingleNode("abc:POOR_IN200901UV/abc:controlActProcess/abc:subject/abc:observationRequest/abc:statusCode/@code").getText());	
			reqno.setMessageId(document.selectSingleNode("abc:POOR_IN200901UV/abc:id/@extension").getText());
			hisamount=document.selectSingleNode("abc:POOR_IN200901UV/abc:controlActProcess/abc:subject/abc:observationRequest/abc:costs/@value").getText();
			reqno.setCode("AA");
		}catch(Exception ex){
				reqno.setCode("AE");
