			...
			if(!lisreq.getPATIENT_ID().equals("") ){
				...
				return sbxml.toString();
					...
					sb.append(" select ci.Id as chargingitemId,ci.item_name,ci.exam_num,d.remark,d.data_name,eci.amount,ci.item_code,eci.item_amount,"
							+ " dd.dep_name as dept_name,eci.id,eci.pay_status,zri.lis_req_code  as sample_barcode,zri.req_id");
					sb.append(" from zl_req_item zri,examinfo_charging_item eci,sample_exam_detail sed,examResult_chargingItem er,sample_demo sd,data_dictionary d,department_dep dd,charging_item  ci"
							+ " where eci.examinfo_id = sed.exam_info_id and eci.charge_item_id = ci.id and sed.sample_id = sd.id"
							+ " and sd.demo_category = d.id and zri.lis_req_code=sed.sample_barcode "
							+ " and ci.sam_demo_id = sd.id "
							+ " and zri.exam_info_id=sed.exam_info_id "
							...
							+ " and eci.change_item != 'C' and eci.pay_status != 'M' and eci.examinfo_id ='"+eu.getId()+"' and zri.zl_pat_id ='"+ eu.getPatient_id()+"'");
//					sb.append(" and dd.dep_inter_num in ('"+requestBody.getEXECDEPTID().replaceAll(",", "','")+"') ");
					...
					if(sendList.size() == 0 && "".equals(noPayItems)){
						lisreq.setCode("AE");
						lisreq.setMessage("没有需要发送的检验项目!!");
					}else if(sendList.size() == 0 && (!"".equals(noPayItems))){
						lisreq.setCode("AE");
						lisreq.setMessage("项目未付费!!");
					}else{
						lisreq.setCode("AA");
						...
						for(LisSendDTOHK305 lissend : sendList){
							...
							sbxml.append("                    <id extension=\""+lissend.getSample_barcode()+"\"/>                                                                          ");
						...
						sbxml.append("            <resultTotalQuantity value=\"1\"/>                                                                              ");
						sbxml.append("        </queryAck>                                                                                                         ");
				...
				lisreq.setCode("AE");
				lisreq.setMessage("申请单 条码 患者id 身份证号为空!!!");
			}
			return sbxml.toString();
		}
		...
		private ExamInfoUserDTO getExamInfoForBarcode( String req_no,String patient_ID) {
			...
			String sql = " select ci.user_name,ci.id_num,ci.sex,ci.birthday,ci.phone,ei.patient_id, "
					...
					+ " from zl_req_item sed, customer_info ci, exam_info ei "
					+ " left join company_info com on com.id = ei.company_id "
					+ " where sed.zl_pat_id='" + patient_ID+ "' and sed.exam_info_id=ei.id and ei.customer_id = ci.id and ei.is_Active='Y' ";
			//此处 需要新增  体检  身份证 查询
