...
public class CUSTOMSendMessageHK305 {
	...
	public CUSTOMSendMessageHK305(Custom custom){
		this.custom=custom;
	}
	...
	private void setSearchString(String url,String statusCode,String logname){
		...
		String exam_num = custom.getEXAM_NUM();
		ExamInfoUserDTO ei = configService.getExamInfoForNum(exam_num);
		custom.setNATION(ei.getNation());
		...
		sb0.append("		<item extension=\""+ei.getExam_num()+"\" identifierName=\"\" root=\"2.16.156.10011.1.13\"/>");//体检(门诊、住院)
		sb0.append("		<item extension=\""+rb1.getPersionid()+"\" identifierName=\"\" root=\"2.16.156.10011.0.2.2\"/>");//患者ID
		sb0.append("	</id> ");
		sb0.append("	<!-- 患者建卡日期(0..1) -->   ");
		sb0.append("	<effectiveDate value=\""+DateTimeUtil.getDateTimes()+"\"/> ");
		...
		String pingYin = getPingYin(custom.getNAME());
		sb0.append("	<!--姓名全拼 (1..1)-->");
		...
		sb0.append("	<!--民族代码(1..*) -->   ");
		sb0.append("	<ethnicGroupCode code=\"\" displayName=\""+custom.getNATION()+"\"/>   ");
			 ...
			 try{
					...
					if(patid.equals("")){
						...
						for (int i = 0; i < subject1List.size(); i++) {
							...
							rb1.setPersionid(subject1List.get(0).selectSingleNode("abc:registrationRequest/abc:subject1/abc:patient/abc:id/abc:item[@root='2.16.156.10011.0.2.2']/@extension").getText());
