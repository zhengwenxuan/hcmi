...
public class SendFeeReqMessageHK305 {
	...
	public ResultHeader getMessage(String url, int days, String logName) {
		String sql = "select distinct ei.* from exam_info ei, customer_info ci ,examinfo_charging_item eci,charging_item c "
				...
				+" and (eci.exam_status ='Y' or c.item_category = '耗材类型') "
...
//			ChargingItem charitem = (ChargingItem) this.jdbcQueryManager.getList(arg0, arg1)load(ChargingItem.class,); 
			...
			//configService.saveChargingSummarySingle(chargingSummarySingle); //保存收费总表
			...
			for(ChargingDetailSingle chargingDetailSingle : detailList){//遍历保存明细数据
				...
				//configService.saveChargingDetailSingle(chargingDetailSingle);
			...
			FEETermSendMessageHK305 fsm= new FEETermSendMessageHK305(fm);		
			...
			System.err.println("===================="+com.hjw.interfaces.util.StringUtil.formatException(e));
		}
		return hrr;
	}
