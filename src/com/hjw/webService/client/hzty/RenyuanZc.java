...
import com.hjw.webService.client.hzty.Bean.RENYUANZCOUT;
 ...
 * 
     ...
     * @version V2.0.0.0
 */
public class RenyuanZc {
	private RENYUANZCIN custom=new RENYUANZCIN();
	
	public RenyuanZc(RENYUANZCIN custom){
		this.custom=custom;
	}
	 ...
	 */
	public RyZcResultBody getMessage(String url,String logname) {
		RyZcResultBody rb = new RyZcResultBody();
		String tradeMsg = "";
		StringHolder tradeMsgOut=new StringHolder();;
		try {
			String tradeType="RENYUANZC";
			...
			try {
				...
				try {
					RENYUANZCOUT rout=new RENYUANZCOUT();
					rout = JaxbUtil.converyToJavaBean(tradeMsgOut.value, RENYUANZCOUT.class);
					if(messages==0){
