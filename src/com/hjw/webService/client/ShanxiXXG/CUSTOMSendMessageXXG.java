...
public class CUSTOMSendMessageXXG {

	private Custom custom=new Custom();
	
	public CUSTOMSendMessageXXG(Custom custom){
		this.custom=custom;
	}
	...
	public ResultBody getMessage(String url,String logname) {
		...
		try {
			...
			try {
				...
				reqUrl.append("/basic/visitsave?head=");
				reqUrl.append(head);
				reqUrl.append("&body=");
				reqUrl.append(body);
...
//				String json = JSONObject.fromObject(reqJson).toString();
				...
				if("1".equals(resbody.getCode())) {
				    ...
				    rb.getResultHeader().setText("error-请求建档服务失败!");
				}
...
//		customer.setPatient_id(custom.getPATIENT_ID());
		customer.setApply_unit(custom.getVISIT_DEPT());
