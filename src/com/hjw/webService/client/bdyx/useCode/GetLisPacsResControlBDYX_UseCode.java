...
public class GetLisPacsResControlBDYX_UseCode {
...
//				TranLogTxt.liswriteEror_to_txt(logname, "body:"+message.getBody());
				if("BS004".equals(message.getHead(1))) {//BS004医嘱执行状态信息服务
					rb = new UpdateStatusBDYX_UseCode().getMessage(message.getBody(), logname);
				} else if("BS319".equals(message.getHead(1))) {//BS319检验报告信息服务
					rb = new GetLisResControlBDYX_UseCode().getMessage(message.getBody(), logname);
				} else if("BS320".equals(message.getHead(1))) {//BS320检查报告信息服务
					rb = new GetPacsResControlBDYX_UseCode().getMessage(message.getBody(), logname);
				} else if("BS354".equals(message.getHead(1))) {//BS354微生物检验报告信息服务
					rb = new GetLisMicrobeResControlBDYX_UseCode().getMessage(message.getBody(), logname);
