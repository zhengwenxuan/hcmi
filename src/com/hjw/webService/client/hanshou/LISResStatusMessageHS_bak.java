 ...
 * 
     ...
     * @version V2.0.0.0
 */
public class LISResStatusMessageHS_bak extends ServletEndpointSupport{
   ...
   static {
	 ...
	 */
	public String UpdateLabApplyInfo(String strbody) {
		...
		rb=getreqNoLisStatus(strbody);
			if("AA".equals(rb.getCode())){
				ExamInfoUserDTO ei = ei=this.getExamInfoForNumLisStatus(rb.getPersionid());
	...
	private ResLisStatusBeanHK getreqNoLisStatus(String xmlstr){
		...
		try{
			...
			reqno.setMessageId(document.selectSingleNode("root/barcodeNo").getText());//条码号
			reqno.setReqno(document.selectSingleNode("root/applyInfos/applyInfo/applyNo").getText());//申请单号
			reqno.setStatus(document.selectSingleNode("root/applyInfos/applyInfo/applyStatus").getText());//申请单状态
